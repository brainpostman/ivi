...
import { IFilterBlockEl, IFilterData, IFilterTitle } from '@/types/filterBlock.interface';
import { useState } from 'react';

export const useFilter = (defaultValue: Omit<IFilterBlockEl, 'isExpand'>[]) => {
    const [filters, setFilters] = useState(defaultValue.map((el) => ({ ...el, isExpand: false })));
