...
/*
  * @param {IBannerCarouselItem[]} items - элементы карусели
  * @param {number} speed - скорость анимации (в мс)
  * @param {boolean} autoscroll - автоскролл
  * @param {number} autoscrollSpeed - скорость автоскролла
  * @param {number} numOfFakes - фейки
  * @returns IUserBannerCarousel
...
export function useBannerCarousel(
  ...
  useEffect(() => {
    ...
    for (
      let i = carouselItems.length - 1;
      i > carouselItems.length - 1 - numOfFakes;
      i--
    ) {
