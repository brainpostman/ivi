...
async function checkAdminRole(accessToken: string | null): Promise<boolean> {
  try {
    const checkAdmin = await axios.get(
      `${process.env.NEXT_PUBLIC_API_PATH}/check-admin`,
      {
...
async function checkEmailVacancy(email: string): Promise<string> {
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_PATH}/check-email/${encodeURIComponent(
        email
      )}`
    )
...
async function signOut(
  ...
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_PATH}/logout/${user_id}`,
      {
        headers: {
