import { ICRUDDetailedFilm, ICRUDFilm } from '@/types/api/ICrudMovie'
...
const AdminMovie = ({ movie, className: propsClassName }: IAdminMovieProps) => {
  ...
  const [detailedMovie, setDetailedMovie] = useState<ICRUDDetailedFilm | null>(
    null
  )
  ...
  const handleShowDetails = async () => {
    setIsLoading(true)
    try {
      const response = await customAxios.get<ICRUDDetailedFilm>(
        `/films/${movie.id}`
      )
