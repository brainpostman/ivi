...
const HoverFilterBlock: FC<IProps> = ({ currentBlock }) => {
    ...
    const blockListStyles = (rowsLength: number) => ({
        gridTemplateColumns: `repeat(${getNumGridColumns(rowsLength, gridColumnLines)}, auto)`,
    });
        ...
        <ul className={style.filterList}>
            {currentBlock?.columns.map((block) => (
                <li key={block.title} className={classNameCurrentBlock(block.title)}>
                    ...
                    <ul className={style.block_list} style={blockListStyles(block.rows.length)}>
                        {block.rows.map((el) => (
                            <li
                                key={el}
                                className={novetly === el ? `text ${style.hoverNovetly}` : 'text'}
