...
const formatWidth = (width: IWidth, elementLens: number[], elementsView: number, space: number) => {
    if (width === 'full') return 1225;
    return elementLens.slice(0, elementsView).reduce((accum, item) => accum + item, 0) - space;
};
...
const CustomCarousel: FC<IProps> = ({
    ...
    const addElementLen = (len: number) => setElementLens((prev) => [...prev, len + space]);
