import { IHeaderTab } from '@/types/header.interface'
import { IHeaderHoverBlockContent } from '@/types/hoverblock.interface'

export const headerHoverBlockContent: IHeaderHoverBlockContent[] = [
	{
		tab: 'films',
		columns: [
			{
				title: 'Жанры',
				rows: [
					'Артхаус',
					'Комедии',
					'Боевики',
					'Криминальные',
					'Вестерн',
					'Мелодрамы',
					'Военные',
					'Мистические',
					'Детективы',
					'По комиксам',
					'Для всей семьи',
					'Приключения',
					'Для детей',
					'Спорт',
					'Документальные',
					'Триллеры',
					'Драмы',
					'Ужасы',
					'Исторические',
					'Фантастика',
					'Катастрофы',
					'Фэнтези',
				],
			},
			{ title: 'Страны', rows: ['Русские', 'Зарубежные', 'Советское кино'] },
			{
				title: 'Годы',
				rows: [
					'Фильмы 2023 года',
					'Фильмы 2022 года',
					'Фильмы 2021 года',
					'Фильмы 2020 года',
				],
			},
			{
				rows: [
					'Новинки',
					'Подборки',
					'Иви.Рейтинг',
					'Скоро на Иви',
					'Трейлеры',
					'Что посмотреть',
					'Фильмы в HD',
					'Выбор на Иви',
					'Новинки подписки',
					'Фильмы Amediateka',
					'Популярные фильмы',
					'Фильмы Иви',
				],
			},
		],
	},
	{
		tab: 'series',
		columns: [
			{
				title: 'Жанры',
				rows: [
					'Биография',
					'Медицинские',
					'Боевики',
					'Мелодрамы',
					'Военные',
					'Мистические',
					'Детективы',
					'Приключения',
					'Для всей семьи',
					'Романтические',
					'Документальные',
					'Телешоу',
					'Дорамы',
					'Триллеры',
					'Драмы',
					'Турецкие',
					'Исторические',
					'Ужасы',
					'Комедийные',
					'Фантастические',
					'Криминальные',
					'Фэнтези',
				],
			},
			{
				title: 'Страны',
				rows: [
					'Русские',
					'Зарубежные',
					'Американские',
					'Украинские',
					'Корейские',
					'Турецкие',
				],
			},
			{
				title: 'Годы',
				rows: [
					'Сериалы 2023 года',
					'Сериалы 2022 года',
					'Сериалы 2021 года',
					'Сериалы 2020 года',
				],
			},
			{
				rows: [
					'Новинки',
					'Сериалы в HD',
					'Бесплатные сериалы',
					'Сериалы Amediateka',
					'Сериалы Paramount Play',
				],
			},
		],
	},
	{
		tab: 'cartoons',
		columns: [
			{
				title: 'Жанры',
				rows: [
					'Аниме',
					'Полнометражные',
					'Боевик',
					'Приключения',
					'Детектив',
					'Развивающие',
					'Для взрослых',
					'Сериалы',
					'Для всей семьи',
					'Спорт',
					'Для детей',
					'Триллер',
					'Драма',
					'Ужасы',
					'История',
					'Фантастика',
					'Комедия',
					'Фэнтези',
					'Криминальные',
					'Мюзикл',
				],
			},
			{
				title: 'Страны',
				rows: ['Советские', 'Русские', 'Американские', 'Зарубежные'],
			},
			{
				title: 'Годы',
				rows: [
					'Мультики 2023 года',
					'Мультики 2022 года',
					'Мультики 2021 года',
					'Мультики 2020 года',
				],
			},
			{
				rows: [
					'Новинки',
					'Мультики в HD',
					'Мультифильы в Paramount Play / Nickelodeon',
					'Мультифильмы Dreamworks',
					'Мультфильмы СТС Kids',
				],
			},
		],
	},
]
