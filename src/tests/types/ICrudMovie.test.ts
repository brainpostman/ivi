...
const crudGenreRequiredProperties = ['id', 'name', 'name_en', 'createdAt', 'updatedAt'];
...
describe('ICRUDMOVIE', () => {
    let genres: ICRUDGenre[];
    let film: ICRUDDetailedMovie;

    beforeAll(async () => {
        genres = await staffsAPI.getCrudGenres();
        film = (await customAxios.get<ICRUDDetailedMovie>(`/films/1`)).data;
    });
